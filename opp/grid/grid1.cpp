...
int main(int argc, char * argv[])
{
	...
	for (int j = 1; j <= sizeX; j++)
	{
		if (0 == rank)
		{
			oldCell[0 * (sizeX + 2) + j] = 0;//top
			cell[0 * (sizeX + 2) + j] = 0;//top
		}

		if (size - 1 == rank)
		{
			oldCell[(sizeY + 1) * (sizeX + 2) + j] = 5;//bottom
			cell[(sizeY + 1) * (sizeX + 2) + j] = 5;//bottom
		}
	}

	for (int i = 1; i <= sizeY; i++)
	{
		oldCell[ i * (sizeX + 2) + 0 ] = 0;//left
		cell[ i * (sizeX + 2) + 0 ] = 0;//left

		oldCell[ i * (sizeX + 2) + sizeX + 1 ] = 0;//right
		cell[ i * (sizeX + 2) + sizeX + 1 ] = 0;//right
	}

	//set second conditions;
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (i == rank)
		{
			...
			if (rank == 0)
			{
				output.open("out1.bin", ofstream::binary);
			}
			else
			{
				output.open("out1.bin", ofstream::binary | ofstream::app);
			}
