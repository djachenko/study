...
int main(int argc, char * argv[])
{
	...
	for (int iteration = 0; ; iteration++)
	{
		...
		for (int i = 2; i <= sizeY - 1; i++)
		{
			for (int j = 1; j <= sizeX; j++)
			{
				...
				if (j == 1)
				{
					cell[index] = oldCell[index + 1] + 0.5;
				}
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (i == rank)
		{
			...
			if (rank == 0)
			{
				output.open("out2.bin", ofstream::binary);
			}
			else
			{
				output.open("out2.bin", ofstream::binary | ofstream::app);
			}
